...
test_that("can read multiple hobo csv file", {
  ...
  expect_is(data, "tbl")
  expect_identical(colnames(data), c("Logger", "DateTime", "Temperature_degC", "FileRow", "FileName", "Directory"))
  expect_identical(nrow(data), 47L)
  expect_identical(unique(data$Logger), sort(c("10171277", "10171279","10171286", "10328118", "10328122", "10723440", "10723450", "2391458")))
  expect_identical(lubridate::tz(data$DateTime), "Etc/GMT+8")
  expect_equal(lubridate::hour(data$DateTime[1:2]), c(13L,13L))
  expect_equal(lubridate::minute(data$DateTime[1:2]), c(3L,18L))
  expect_equal(data$Temperature_degC[1:2], c(4.947, 3.406))
})
