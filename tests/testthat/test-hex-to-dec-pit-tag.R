...
test_that("output is 15 digits long when dec outputted is 12 digits long", {
  dec_code <- hex_to_dec_pit_tag("349EA72A50")
  expect_identical(dec_code, 900226000054864)
  expect_length(dec_code, 15L)
})

test_that("output is 15 digits long when dec outputted is 11 digits long", {
...
test_that("output is NA when dec outputted more then 12 digits long", {
  ...
  expect_identical(dec_code, NA_real_)
})
